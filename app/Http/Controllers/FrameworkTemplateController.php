...
class FrameworkTemplateController extends Controller
{
    ...
    */
    ...
    public function publicIndex(Request $request)
    {
        ...
        $filePath = resource_path('views/aspiaUcl/frameworks/frameworks_overview.blade.php');
    ...
    */
    ...
    public function show(string $slug)
    {
        ...
        if (!$template || empty(trim($template->html_content ?? ''))) {
            $template = FrameworkTemplate::whereNotNull('html_content')
                ->where('html_content', '!=', '')
                ->latest('updated_at')
                ->first();
        }
        ...
        if (empty(trim($html ?? ''))) {
            return redirect()
                ->back()
                ->with('error', "HTML template is missing in database for framework '{$framework->name}'. Please insert an HTML template first.");
        }
...
HTML;
    ...
    private function renderMissingTemplateError(string $moduleName, string $recordName, ?string $type = null, ?string $dashboardUrl = null)
    {
        ...
        $typeNotice = $type ? " for framework type <strong>\"" . e($type) . "\"</strong>" : "";
