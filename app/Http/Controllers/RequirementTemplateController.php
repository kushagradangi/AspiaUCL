...
class RequirementTemplateController extends Controller
{
    ...
    */
    ...
    public function store(Request $request)
    {
        ...
        if ($template) {
            ...
            RequirementTemplate::where('id', '!=', $template->id)->delete();
        } else {
    ...
    */
    ...
    public function show($requirement_id)
    {
        ...
        if (!$requirement) {
            $requirement = Requirement::with('control.domain.framework')
                ->whereRaw('LOWER(requirement_id) = ?', [strtolower(trim($requirement_id))])
                ->orWhere('id', $requirement_id)
                ->firstOrFail();
        }
        ...
        $controlCriticality       = $control?->criticality ?? 'High';
        $controlIndustries        = $control?->applicable_industries ?? 'All Industries';
        $controlTechnologies      = $control?->applicable_technologies ?? 'Enterprise IT & Cloud Infrastructure';
        $controlStatus            = $control?->status ?? 'Active';
        $controlVersion           = $control?->version ?? '1.0';
        $controlBenefits          = $control?->business_benefits ?? '';
        $controlRisksIfMissing    = $control?->business_risks_if_missing ?? '';
        $controlStakeholders      = $control?->primary_stakeholders ?? '';
        $controlUrl               = $control ? route('controls.show', $control->control_id) : '#';

        $controlBadge = $control ? <<<HTML
        <a href="{$controlUrl}" class="badge-chip chip-purple" title="View Control {$controlId}">
            <span>Control: {$controlId}</span>
            <span style="opacity: 0.7; font-size: 10px;">↗</span>
        </a>
HTML : "<span class=\"badge-chip chip-purple\">Control: {$controlId}</span>";
        ...
        <a href="{$domainUrl}" class="badge-chip chip-cyan" title="View Domain {$domainCode}">
            <span>Domain: {$domainCode}</span>
            <span style="opacity: 0.7; font-size: 10px;">↗</span>
        </a>
HTML : ($domainName ? "<span class=\"badge-chip chip-cyan\">Domain: {$domainName}</span>" : '');
        ...
        // Framework Mappings
        $mappings                = $requirement->frameworkMappings()->get();
        $mappingsCount           = $mappings->count();
        $frameworkMappingsBadges = $this->renderFrameworkMappingsBadges($requirement, $mappings);
        $frameworkMappingsTable  = $this->renderFrameworkMappingsTable($requirement, $mappings);
        $frameworkBadge          = $frameworkMappingsBadges;

        // Sibling Requirements under same Control
        $siblingRequirements      = $control ? $control->requirements()->orderBy('display_order', 'asc')->orderBy('id', 'asc')->get() : collect([$requirement]);
        $siblingRequirementsCount = $siblingRequirements->count();
        $siblingRequirementsChips = $this->renderSiblingRequirementsChips($siblingRequirements, $requirement->requirement_id);
        $siblingRequirementsTable = $this->renderSiblingRequirementsTable($siblingRequirements, $requirement->requirement_id);

        $canonicalUrl = route('requirements.show', $requirement->requirement_id);
        ...
        $placeholders = [
            ...
            '{{applicable_industries}}'     => $controlIndustries,
            '{{applicable_technologies}}'   => $controlTechnologies,
            '{{status}}'                    => $controlStatus,
            '{{version}}'                   => $controlVersion,
            '{{business_benefits}}'         => $controlBenefits,
            '{{business_risks_if_missing}}' => $controlRisksIfMissing,
            '{{primary_stakeholders}}'      => $controlStakeholders,
            ...
            '{{framework_mappings}}'         => $frameworkMappingsBadges,
            '{{framework_mappings_badges}}'  => $frameworkMappingsBadges,
            '{{framework_mappings_table}}'   => $frameworkMappingsTable,
            '{{mappings_count}}'             => $mappingsCount,
            '{{sibling_requirements_chips}}' => $siblingRequirementsChips,
            '{{sibling_requirements_table}}' => $siblingRequirementsTable,
            '{{sibling_requirements_count}}' => $siblingRequirementsCount,
        ];
    ...
    protected function renderFrameworkMappingsBadges($requirement, $mappings = null): string
    {
        $mappings = $mappings ?? $requirement->frameworkMappings()->get();
        if ($mappings->isEmpty()) {
            return '<span class="badge-chip chip-purple font-mono">Universal Control Library</span>';
        }
        ...
        foreach ($mappings as $m) {
            ...
            $fwName = htmlspecialchars($m->framework_name);
            $clause = htmlspecialchars($m->clause_reference ?? '');

            $chips .= <<<HTML
            <a href="{$url}" class="badge-chip chip-cyan" title="View {$fwName}">
                <span><strong>{$fwName}</strong>: {$clause}</span>
                <span style="opacity: 0.7; font-size: 10px;">↗</span>
            </a>
HTML;
        }

        return "<div style=\"display: flex; flex-wrap: wrap; gap: 8px; margin-top: 6px;\">{$chips}</div>";
    }

    protected function renderFrameworkMappingsTable($requirement, $mappings = null): string
    {
        $mappings = $mappings ?? $requirement->frameworkMappings()->get();
        if ($mappings->isEmpty()) {
            return '<p style="color: var(--text-muted); font-size: 13.5px; padding: 12px 0;">No direct framework mappings configured for this requirement yet.</p>';
        }
        ...
        foreach ($mappings as $m) {
            ...
            $url = $fwModel ? route('frameworks.show', $fwModel->slug) : '#';
            $fwName = htmlspecialchars($m->framework_name);
            $code   = htmlspecialchars($m->framework_code ?? $fwModel?->framework_code ?? '-');
            $clause = htmlspecialchars($m->clause_reference ?? '-');

            $rows .= <<<HTML
            <tr>
                <td style="font-weight: 700;">
                    <a href="{$url}" style="color: var(--accent-cyan); text-decoration: none;">{$fwName}</a>
                </td>
                <td><span class="badge-chip chip-purple font-mono" style="font-size: 11px;">{$code}</span></td>
                <td><strong class="font-mono" style="color: var(--accent-emerald);">{$clause}</strong></td>
                <td style="text-align: right;">
                    <a href="{$url}" class="badge-chip chip-cyan" style="font-size: 11px; padding: 4px 10px;">View Framework →</a>
                </td>
            </tr>
HTML;
        }
        ...
        <div class="table-responsive">
            <table>
                <thead>
                    <tr>
                        <th style="width: 32%;">Compliance Framework</th>
                        <th style="width: 20%;">Code</th>
                        <th style="width: 28%;">Clause / Section Reference</th>
...
HTML;
    ...
    protected function renderSiblingRequirementsChips($requirements, string $currentRequirementId): string
    {
        if ($requirements->isEmpty()) {
            return '<p style="color: var(--text-muted); font-size: 13px;">No sibling requirements under this control.</p>';
        }
        ...
        foreach ($requirements as $req) {
            ...
            $isCurrent = (strcasecmp($req->requirement_id, $currentRequirementId) === 0);

            $chipClass = $isCurrent ? 'chip-cyan' : 'chip-purple';
            $activeStyle = $isCurrent ? 'outline: 2px solid var(--accent-cyan); font-weight: 800;' : '';

            $items .= <<<HTML
            <a href="{$reqUrl}" class="badge-chip {$chipClass}" style="{$activeStyle}" title="{$title}">
                <span><strong>{$rid}</strong>: {$title}</span>
                <span style="opacity: 0.7; font-size: 10px;">↗</span>
            </a>
HTML;
        }
    ...
    protected function renderSiblingRequirementsTable($requirements, string $currentRequirementId): string
    {
        if ($requirements->isEmpty()) {
            return '<p style="color: var(--text-muted); font-size: 13px;">No sibling requirements under this control.</p>';
        }
        ...
        foreach ($requirements as $req) {
            ...
            $rid    = htmlspecialchars($req->requirement_id);
            $title  = htmlspecialchars($req->requirement_title);
            $owner  = htmlspecialchars($req->typical_owner ?? 'InfoSec Team');
            $isCurrent = (strcasecmp($req->requirement_id, $currentRequirementId) === 0);

            $highlightStyle = $isCurrent ? 'background: var(--accent-cyan-glow);' : '';
            $badge = $isCurrent ? '<span class="badge-chip chip-emerald" style="font-size: 10px; padding: 2px 8px; margin-left: 6px;">Current</span>' : '';

            $rows .= <<<HTML
            <tr style="{$highlightStyle}">
                <td>
                    <a href="{$reqUrl}" style="color: var(--accent-cyan); font-weight: 700; text-decoration: none;" class="font-mono">{$rid}</a>
                    {$badge}
                </td>
                <td><a href="{$reqUrl}" style="color: var(--text-primary); text-decoration: none; font-weight: 600;">{$title}</a></td>
                <td><span style="color: var(--text-secondary);">{$owner}</span></td>
                <td style="text-align: right;">
                    <a href="{$reqUrl}" class="badge-chip chip-cyan" style="font-size: 11px; padding: 4px 10px;">View Requirement →</a>
                </td>
            </tr>
HTML;
        }
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 45%;">Requirement Title</th>
...
<head>
    ...
    <title>{{requirement_title}} ({{requirement_id}}) | ASPIA Unified Control Library</title>
    <meta name="description" content="Auditable compliance specification, implementation guidance, common audit findings, best practices, and cross-framework mappings for {{requirement_title}} ({{requirement_id}}) under control {{control_name}} ({{control_id}}).">
    <meta name="keywords" content="{{requirement_id}}, {{requirement_title}}, {{control_id}}, {{control_name}}, {{domain_name}}, {{domain_code}}, {{framework_name}}, cybersecurity compliance, audit findings, implementation guidance, UCL, ASPIA">
    ...
    <meta property="og:title" content="{{requirement_id}}: {{requirement_title}} | ASPIA UCL">
    <meta property="og:description" content="Compliance requirements, audit findings, implementation guidance, and regulatory mappings for {{requirement_title}} ({{requirement_id}}).">
    ...
    <meta name="twitter:title" content="{{requirement_id}}: {{requirement_title}} | ASPIA UCL">
    <meta name="twitter:description" content="Enforcement requirement specifications, implementation steps, and regulatory mappings for {{requirement_id}}.">
        ...
        "@graph": [
            {
                ...
                "headline": "{{requirement_title}} ({{requirement_id}})",
                "description": "Auditable compliance requirement specifications and implementation guidance under Control {{control_id}} ({{control_name}}).",
                "articleSection": "{{domain_name}}",
                "inLanguage": "en-US",
                "about": {
                    "@type": "DefinedTerm",
                    "name": "{{requirement_title}}",
                    "termCode": "{{requirement_id}}",
                    "inDefinedTermSet": {
                        "@type": "DefinedTermSet",
                        "name": "{{control_name}} ({{control_id}})",
                        "url": "{{control_url}}"
                    }
                }
            },
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .breadcrumb-sep { opacity: 0.5; font-size: 11px; }
        .breadcrumb-current { color: var(--accent-cyan); font-weight: 700; }
        ...
        .requirement-h1 { font-size: 32px; font-weight: 800; color: var(--text-primary); letter-spacing: -0.6px; line-height: 1.25; margin-bottom: 16px; }
        ...
        .meta-card {
            display: flex; align-items: flex-start; gap: 12px; background: var(--bg-primary);
        ...
        .meta-icon-box {
            ...
            font-size: 16px; background: var(--bg-secondary); border: 1px solid var(--border-color); flex-shrink: 0; margin-top: 2px;
        }

        .meta-info-col { display: flex; flex-direction: column; gap: 2px; min-width: 0; width: 100%; }
        .meta-caption { font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.7px; color: var(--text-muted); font-weight: 800; }
        .meta-val { font-size: 13.5px; font-weight: 700; color: var(--text-primary); white-space: normal; word-break: break-word; line-height: 1.45; }
        ...
        .stat-value { font-size: 30px; font-weight: 800; color: var(--text-primary); font-family: 'JetBrains Mono', monospace; line-height: 1.1; white-space: nowrap; margin-top: 4px; }
        .stat-icon-bubble { width: 48px; height: 48px; border-radius: 16px; display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0; }
        ...
        /* Callout & Narrative Blocks */
        .callout-block { background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: var(--radius-lg); padding: 22px 24px; margin-bottom: 20px; }
        ...
        .callout-block.accent-emerald { border-left: 4px solid var(--accent-emerald); }
        .callout-block.accent-amber { border-left: 4px solid var(--accent-amber); }
        .callout-block.accent-rose { border-left: 4px solid var(--accent-rose); }
        ...
        .narrative-card.audit-border { border-left: 4px solid var(--accent-amber); }
        ...
        table th:first-child, table td:first-child, table td:first-child a { white-space: nowrap !important; word-break: keep-all !important; hyphens: none !important; min-width: 150px !important; }
        table th:last-child, table td:last-child { white-space: nowrap !important; min-width: 120px !important; text-align: right !important; }
...
<body>
    <!-- Top Navigation & Dark-Light Switch -->
    <header class="top-toolbar">
        <nav class="breadcrumbs-nav" aria-label="Breadcrumbs">
            <a href="/" class="breadcrumb-item">ASPIA UCL</a>
            <span class="breadcrumb-sep">/</span>
            <a href="{{framework_url}}" class="breadcrumb-item">{{framework_code}}</a>
            <span class="breadcrumb-sep">/</span>
            <a href="{{domain_url}}" class="breadcrumb-item">{{domain_code}}</a>
            <span class="breadcrumb-sep">/</span>
            <a href="{{control_url}}" class="breadcrumb-item">{{control_id}}</a>
            <span class="breadcrumb-sep">/</span>
            <span class="breadcrumb-current font-mono">{{requirement_id}}</span>
        </nav>
    ...
    <!-- Requirement Main Hero Card -->
    <article class="hero-card">
        <div class="tags-row">
            <span class="badge-chip chip-cyan font-mono" id="reqIdBadge">◈ {{requirement_id}}</span>
            {{control_badge}}
            {{domain_badge}}
            <span class="badge-chip chip-emerald" style="white-space: normal; word-break: break-word; text-align: left;">👤 Owner: {{typical_owner}}</span>
        </div>

        <h1 class="requirement-h1">{{requirement_title}}</h1>

        <div class="statement-card">
            <div class="statement-label">
                <span>📌</span>
                <span>Official Auditable Requirement Statement</span>
            </div>
            <p class="statement-body">{{requirement}}</p>
        </div>

        <div class="meta-cards-grid">
            <div class="meta-card">
                <div class="meta-icon-box" style="color: var(--accent-purple);">🛡️</div>
                <div class="meta-info-col">
                    <span class="meta-caption">Parent Control</span>
                    <span class="meta-val">
                        <a href="{{control_url}}" style="color: var(--accent-purple); text-decoration: none;">{{control_id}}: {{control_name}}</a>
                    </span>
                </div>
            </div>
            <div class="meta-card">
                <div class="meta-icon-box" style="color: var(--accent-cyan);">◈</div>
                <div class="meta-info-col">
                    <span class="meta-caption">Domain</span>
                    <span class="meta-val">
                        <a href="{{domain_url}}" style="color: var(--accent-cyan); text-decoration: none;">{{domain_name}} ({{domain_code}})</a>
                    </span>
                </div>
            </div>
            <div class="meta-card">
                <div class="meta-icon-box" style="color: var(--accent-emerald);">⚙️</div>
                <div class="meta-info-col">
                    <span class="meta-caption">Control Category</span>
                    <span class="meta-val" style="color: var(--accent-emerald);">{{control_category}}</span>
                </div>
            </div>
            <div class="meta-card">
                <div class="meta-icon-box" style="color: var(--accent-amber);">🏛️</div>
                <div class="meta-info-col">
                    <span class="meta-caption">Parent Framework</span>
                    <span class="meta-val">
                        <a href="{{framework_url}}" style="color: var(--accent-amber); text-decoration: none;">{{framework_name}} ({{framework_code}})</a>
                    </span>
                </div>
            </div>
        </div>
    ...
    <!-- Key Requirement Statistics Row -->
    <section class="stats-grid" aria-label="Requirement Statistics">
        <div class="stat-card stat-card-cyan" onclick="switchTab('frameworks-tab')">
            <div class="stat-content">
                <span class="stat-label">Mapped Frameworks</span>
                <span class="stat-value" style="color: var(--accent-cyan);">{{mappings_count}}</span>
            </div>
            <div class="stat-icon-bubble chip-cyan">🌐</div>
        </div>
        <div class="stat-card stat-card-purple" onclick="switchTab('control-tab')">
            <div class="stat-content">
                <span class="stat-label">Parent Control</span>
                <span class="stat-value" style="color: var(--accent-purple); font-size: 22px;">{{control_id}}</span>
            </div>
            <div class="stat-icon-bubble chip-purple">🛡️</div>
        </div>
        <div class="stat-card stat-card-emerald" onclick="switchTab('siblings-tab')">
            <div class="stat-content">
                <span class="stat-label">Control Siblings</span>
                <span class="stat-value" style="color: var(--accent-emerald);">{{sibling_requirements_count}}</span>
            </div>
            <div class="stat-icon-bubble chip-emerald">◆</div>
        </div>
    </section>

    <!-- Main Content Panel & Interactive Tabs -->
    <main class="content-panel">
        <div class="tabs-header">
            <nav class="tabs-nav" role="tablist">
                <button class="tab-btn active" id="btn-details-tab" onclick="switchTab('details-tab')" role="tab" aria-selected="true">
                    <span>📋 Requirement Details & Guidance</span>
                </button>
                <button class="tab-btn" id="btn-control-tab" onclick="switchTab('control-tab')" role="tab" aria-selected="false">
                    <span>🛡️ Associated Control Records</span>
                </button>
                <button class="tab-btn" id="btn-frameworks-tab" onclick="switchTab('frameworks-tab')" role="tab" aria-selected="false">
                    <span>🌐 Framework Mappings</span>
                    <span class="tab-counter">{{mappings_count}}</span>
                </button>
                <button class="tab-btn" id="btn-siblings-tab" onclick="switchTab('siblings-tab')" role="tab" aria-selected="false">
                    <span>◈ Domain & Sibling Context</span>
                </button>
            </nav>
        ...
        <!-- TAB 1: Requirement Details & Guidance (All 11 Excel Record Fields) -->
        <section class="tab-content active" id="details-tab" role="tabpanel">
            <!-- 1. Top Metadata Specification Bar (Cols A, B, C, K) -->
            <div class="section-box">
                <div class="meta-cards-grid" style="padding-top: 0; border-top: none; margin-bottom: 24px;">
                    <div class="meta-card">
                        <div class="meta-icon-box" style="color: var(--accent-cyan);">◈</div>
                        <div class="meta-info-col">
                            <span class="meta-caption">Requirement ID</span>
                            <span class="meta-val font-mono" style="color: var(--accent-cyan);">{{requirement_id}}</span>
                        </div>
                    </div>
                    <div class="meta-card">
                        <div class="meta-icon-box" style="color: var(--accent-purple);">🛡️</div>
                        <div class="meta-info-col">
                            <span class="meta-caption">Control ID</span>
                            <span class="meta-val">
                                <a href="{{control_url}}" style="color: var(--accent-purple); text-decoration: none;" class="font-mono">{{control_id}} ↗</a>
                            </span>
                        </div>
                    </div>
                    <div class="meta-card">
                        <div class="meta-icon-box" style="color: var(--accent-cyan);">📋</div>
                        <div class="meta-info-col">
                            <span class="meta-caption">Requirement Title</span>
                            <span class="meta-val">{{requirement_title}}</span>
                        </div>
                    </div>
                    <div class="meta-card">
                        <div class="meta-icon-box" style="color: var(--accent-emerald);">👤</div>
                        <div class="meta-info-col">
                            <span class="meta-caption">Typical Owner</span>
                            <span class="meta-val">{{typical_owner}}</span>
                        </div>
                    </div>
                </div>
            </div>

            <!-- 2. Requirement Statement (Col D) -->
            <div class="section-box">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-cyan);">📌</span> Requirement
                </h2>
                <div class="statement-card" style="margin-bottom: 0;">
                    <div class="statement-label">
                        <span>Official Auditable Requirement Statement</span>
                    </div>
                    <p class="statement-body">{{requirement}}</p>
            ...
            <!-- 3. Why this Requirement Exists (Col E) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-purple);">💡</span> Why this Requirement Exists
                </h2>
                <div class="callout-block accent-purple">
                    <p class="callout-text">{{why_requirement_exists}}</p>
                </div>
            </div>

            <!-- 4. Implementation Guidance (Col F) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-emerald);">🛠️</span> Implementation Guidance
                </h2>
                <div class="callout-block accent-emerald">
                    <p class="callout-text">{{implementation_guidance}}</p>
                </div>
            </div>

            <!-- 5. Business Examples (Col J) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-cyan);">💼</span> Business Examples
                </h2>
                <div class="callout-block accent-cyan">
                    <p class="callout-text">{{business_examples}}</p>
                </div>
            </div>

            <!-- 6. Common Audit Findings (Col G) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-amber);">⚠️</span> Common Audit Findings
                </h2>
                <div class="callout-block accent-amber">
                    <p class="callout-text">{{common_audit_findings}}</p>
                </div>
            </div>

            <!-- 7. Common Mistakes (Col H) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-rose);">🚫</span> Common Mistakes
                </h2>
                <div class="callout-block accent-rose">
                    <p class="callout-text">{{common_mistakes}}</p>
                </div>
            </div>

            <!-- 8. Best Practices (Col I) -->
            <div class="section-box" style="margin-top: 28px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-emerald);">⭐</span> Best Practices
                </h2>
                <div class="callout-block accent-emerald">
                    <p class="callout-text">{{best_practices}}</p>
                </div>
            </div>
        </section>

        <!-- TAB 2: Associated Control Records -->
        <section class="tab-content" id="control-tab" role="tabpanel">
            <div class="section-box">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-purple);">🛡️</span> Parent Control Overview
                </h2>
                <div class="callout-block accent-purple">
                    <h3 class="callout-title" style="color: var(--accent-purple);">
                        <span>📌</span> {{control_id}}: {{control_name}}
                    </h3>
                    <p class="callout-text">{{control_summary}}</p>
                </div>
                <div class="callout-block accent-cyan">
                    <h3 class="callout-title" style="color: var(--accent-cyan);">
            ...
            <div class="section-box" style="margin-top: 32px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-emerald);">🎯</span> Business Objectives & Risk Mitigations
                </h2>
                ...
                <div class="table-responsive">
                    <table>
                        <tbody>
                            ...
                            <tr><th>15. Control Profile Link</th><td><a href="{{control_url}}" class="badge-chip chip-cyan" style="font-size: 11px; padding: 4px 12px;">View Complete Control Page →</a></td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <!-- TAB 3: Framework Information & Cross-Mappings -->
        <section class="tab-content" id="frameworks-tab" role="tabpanel">
            <!-- 1. Parent Framework -->
            <div class="section-box">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-cyan);">◇</span> Parent Framework (Native Origin)
                </h2>
                <div class="callout-block accent-cyan" style="margin-bottom: 18px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
                        <div>
                            <h3 class="callout-title" style="color: var(--accent-cyan); margin-bottom: 4px;">
                                <span>🏛️</span> {{framework_name}} <span class="badge-chip chip-cyan font-mono" style="font-size: 11px; margin-left: 8px;">{{framework_code}}</span>
                            </h3>
                            <p class="callout-text" style="font-size: 13.5px;">Primary origin framework under which this requirement and its parent control are structured.</p>
                        </div>
                        <a href="{{framework_url}}" class="badge-chip chip-cyan" style="font-size: 12px; padding: 6px 16px;">
                            <span>View Full Framework ({{framework_code}})</span>
                            <span>↗</span>
                        </a>
                    </div>
                </div>
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{framework_url}}" class="badge-chip chip-cyan" style="font-size: 12px;">
                                        <span>View Full Framework ({{framework_code}})</span>
                                        <span>↗</span>
                                    </a>
                                </td>
            ...
            <!-- 2. Cross-Framework Compliance Mappings -->
            <div class="section-box" style="margin-top: 36px;">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-cyan);">🌐</span> Cross-Framework Compliance Mapping Matrix ({{mappings_count}})
                </h2>
                <p style="color: var(--text-muted); font-size: 13.5px; margin-bottom: 16px;">
                    This requirement satisfies compliance obligations across the following international cybersecurity standards, data privacy laws, and regulatory benchmarks:
                </p>
                <div style="margin-bottom: 20px;">
                    {{framework_mappings_badges}}
                </div>
                {{framework_mappings_table}}
            </div>
        </section>

        <!-- TAB 5: Domain & Sibling Context -->
        <section class="tab-content" id="siblings-tab" role="tabpanel">
            <div class="section-box">
                <h2 class="section-header-title">
                    <span style="color: var(--accent-emerald);">◆</span> Sibling Requirements under {{control_id}} ({{sibling_requirements_count}})
                </h2>
                <p style="color: var(--text-muted); font-size: 13.5px; margin-bottom: 14px;">
                    Other compliance enforcement requirements grouped under the same parent control:
                </p>
                {{sibling_requirements_table}}
            </div>
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{domain_url}}" class="badge-chip chip-purple" style="font-size: 12px;">
