...
HTML;
    ...
    */
    ...
    public function show(string $control_id)
    {
        ...
        $template = ControlTemplate::whereNotNull('html_content')
            ->where('html_content', '!=', '')
            ->latest('updated_at')
            ->first();
        ...
        if (empty(trim($html ?? ''))) {
            return redirect()
                ->back()
                ->with('error', "HTML template is missing in database for control '{$control->control_id}'. Please insert an HTML template first.");
        }
...
HTML : ($domainCode ? "<span class=\"badge badge-purple\">View</span>" : '');
