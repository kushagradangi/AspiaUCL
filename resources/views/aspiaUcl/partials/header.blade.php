        ...
        <nav class="nav" aria-label="Homepage Navigation">
            ...
            <ul class="nav-links">
                <li class="dropdown-parent">
                    <a href="{{ route('frameworks.public_index') }}" class="{{ ($activeTab ?? '') === 'frameworks' ? 'active' : '' }}" title="Compliance Frameworks">
                    ...
                    <div class="dropdown-popover">
                        <div class="dropdown-popover-header">
                            ...
                            <a href="{{ route('frameworks.public_index') }}" class="popover-all-link">View All →</a>
                        </div>
                        <div class="dropdown-popover-list">
                            @foreach($headerFrameworks as $fw)
                                @php
                                ...
                                @endphp
                                <a href="{{ $url }}" class="dropdown-card">
                                    <div class="dropdown-card-body">
                                        <div class="dropdown-card-title-row">
                                            <span class="dropdown-card-title">{{ $name }}</span>
                                            <span class="card-badge {{ $badgeClass }}">{{ $code }}</span>
                                        </div>
                                        <span class="dropdown-card-sub">{{ $cat }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                ...
                <li class="dropdown-parent">
                    <a href="{{ route('domains.public_index') }}" class="{{ ($activeTab ?? '') === 'domains' ? 'active' : '' }}" title="Governance Domains">
                        <span>Domains</span>
                        <i class="fas fa-chevron-down dropdown-arrow" aria-hidden="true"></i>
                    </a>
                    <div class="dropdown-popover">
                        <div class="dropdown-popover-header">
                            ...
                            <a href="{{ route('domains.public_index') }}" class="popover-all-link">View All →</a>
                        </div>
                        <div class="dropdown-popover-list">
                            @foreach($headerDomains as $d)
                                @php
                                    $badgeClass = $badgeClasses[$loop->index % count($badgeClasses)];
                                ...
                                <a href="{{ $url }}" class="dropdown-card">
                                    <div class="dropdown-card-body">
                                        ...
                                        <span class="dropdown-card-sub">{{ $sub }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                ...
                <li class="dropdown-parent">
                    <a href="{{ route('controls.public_index') }}" class="{{ ($activeTab ?? '') === 'controls' ? 'active' : '' }}" title="Unified Controls">
                        <span>Controls</span>
                        <i class="fas fa-chevron-down dropdown-arrow" aria-hidden="true"></i>
                    </a>
                    <div class="dropdown-popover">
                        <div class="dropdown-popover-header">
                            ...
                            <a href="{{ route('controls.public_index') }}" class="popover-all-link">View All →</a>
                        </div>
                        <div class="dropdown-popover-list">
                            @foreach($headerControls as $c)
                                @php
                                    $badgeClass = $badgeClasses[$loop->index % count($badgeClasses)];
            ...
            <div class="nav-actions">
                @auth
                    <a href="{{ route('dashboard') }}" class="btn-outline" title="Go to Dashboard"><i class="fas fa-chart-line" aria-hidden="true"></i> Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="btn-outline" title="Sign In">Sign In</a>
                @endauth
...
<header id="stickyLightNavbar" class="sticky-light-navbar" aria-label="Sticky Navigation Bar">
    <div class="container">
        <nav class="sticky-nav">
            <!-- Brand Logo -->
            <a href="{{ route('home') }}" class="sticky-logo" title="ASPIA UCL Homepage">
            ...
            <div class="sticky-nav-actions">
                @auth
                    <a href="{{ route('dashboard') }}" class="sticky-btn-signin" title="Go to Dashboard">
                        <i class="fas fa-chart-line" aria-hidden="true"></i> Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}" class="sticky-btn-signin" title="Sign In">Sign In</a>
                @endauth
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const searchIndex = [
        @foreach($headerFrameworks as $fw)
            { name: "{{ addslashes($fw->name) }}", code: "{{ addslashes($fw->framework_code) }}", type: "Framework", url: "{{ route('frameworks.show', $fw->slug) }}" },
        @endforeach
        @foreach($headerDomains as $d)
            { name: "{{ addslashes($d->name) }}", code: "{{ addslashes($d->domain_code ?: $d->domain_id) }}", type: "Domain", url: "{{ route('domains.show', $d->slug) }}" },
        @endforeach
        @foreach($headerControls as $c)
            { name: "{{ addslashes($c->name) }}", code: "{{ addslashes($c->control_id) }}", type: "Control", url: "{{ route('controls.show', $c->control_id) }}" },
        @endforeach
    ];
